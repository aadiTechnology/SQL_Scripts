/****** Object:  StoredProcedure [dbo].[usp_GetUserRolewisePPSIdentyCard]    Script Date: 19-06-2026 10:03:32 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[usp_GetUserRolewisePPSIdentyCard]
	@School_Id INT,
...
BEGIN
	...
	BEGIN
			...
			SELECT UM.User_Role_Id,
				   ...
				   UBD.EmployeeNo,
				   BTD.Mobile_Number,
				   BTD.Local_Address+', '+btd.Local_City+' '+CAST(btd.Local_Pincode AS NVARCHAR(10)), 
				   UM.EmergencyContactNumber,
				   BTD.Date_of_Birth,
				   UBD.JoiningDate,
				   ISNULL(BG.BloodGroup,''),
	...
	BEGIN
			...
			SELECT UM.User_Role_Id,
				   ...
				   UBD.EmployeeNo,
				   BSD.Mobile_Number,
				   UM.Address+', '+ SSM.City + ' '+CAST(SSM.Pincode AS NVARCHAR(10)),				   
				   UM.EmergencyContactNumber,				   
				   UM.DOB,				   
				   UBD.JoiningDate,
				   ISNULL(BG.BloodGroup,''),
				   TDM.SortOrder,
				   SSM.BinaryPhotoImage
				  -- UM.BinaryPhotoImage
			...
			ON UBD.BloodGroupId = BG.Id
			AND BG.IsDeleted = 0
			INNER JOIN SchoolWise_Supervisor_Master SSM
			ON BSD.Supervisor_Id = SSM.Supervisor_Id
	...
	SET PersonAddress = REPLACE(REPLACE(REPLACE(PersonAddress,CHAR(13)+CHAR(10),' '),CHAR(13),' '),CHAR(10),' ')
	...
	SELECT ROW_NUMBER() OVER (ORDER BY UserRoleId, DesignationSortOrder) as RowNo,
		...
		'Tel : '+SM.Phone_Number + CASE WHEN SM.Phone_Number2 is not null AND SM.Phone_Number2 <> '' THEN ' / '+RIGHT(SM.Phone_Number2,3) ELSE '' END AS SchoolContactNo,
	...
	AND BinaryPhotoImage IS NOT NULL
END